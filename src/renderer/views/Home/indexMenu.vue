            ...
            {{$t('home.Latest_Transaction_local')}}
                ...
                <Submenu name="other">
                    ...
                    <MenuGroup :title="$t('home.Withdraw')">
                        ...
                        <MenuItem v-if="$role('home.tx.Withdrawprofit')" name="openWithdrawprofit">
                            {{ $t('home.Withdrawprofit') }}
                        </MenuItem>
                        <MenuItem v-if="$role('home.tx.Minerprofit')" name="openMinerprofit">
                            {{ $t('somemodel.Extractstorageandmininrewards') }}
                        </MenuItem>
                        <MenuItem v-if="$role('home.tx.Minerprofit')" name="openMinerOrder">
                            {{ $t('orderrevenue.WithdraworderCommission') }}
                        </MenuItem>

                        <MenuItem v-if="$role('home.tx.Minerprofit')" name="openAuthorizedMarket">
                            {{ $t('Assetincomepop.title') }}
                        </MenuItem>
                    </MenuGroup>
                </Submenu>
            ...
            {{$t('home.miningguide')}}
                ...
                <Submenu v-if="$role('home.Myorderlist')" name="myorderlist">
                    ...
                    <MenuGroup :title="$t('assetsotherpage.lambdaTitle')">
                        <MenuItem to="/home/myorderlist" name="myorderlist1">
                            {{ $t('assetsotherpage.menulist.Lambdamarketorder') }}
                        </MenuItem>
                    </MenuGroup>
                    <MenuGroup :title="$t('assetsotherpage.assetTitle')">
                        <MenuItem to="/home/assetsorderlist" name="myorderlist2">
                            {{ $t('assetsotherpage.menulist.Assetmarketorder') }}
                        </MenuItem>
                        <MenuItem to="/home/acceptOrder" name="myorderlist3">
                            {{ $t('assetsotherpage.menulist.documentsaccepted') }}
                        </MenuItem>
                    </MenuGroup>
                </Submenu>
        ...
        transferClick(name) {
            ...
            switch (name) {
                case 'openTransfer':
                    ...
                    this.$data.activeItem = this.$route.name;
                    break;
                case 'openWithdraw':
                    this.$refs.WithdrawalModalDialog.open();
                    this.$data.activeItem = this.$route.name;
                    break;
                case 'openWithdrawprofit':
                    this.$refs.DistributionModal.open();
                    this.$data.activeItem = this.$route.name;
                    break;
                case 'openMinerprofit':
                ...
                case 'openMinerOrder':
                    ...
                    this.$data.activeItem = this.$route.name;
                    break;
                case 'openWithdrawmarket':
                    this.$refs.WithdrawalMarketModalDialog.open();
                    this.$data.activeItem = this.$route.name;
                    break;

                case 'openAuthorizedMarket':
