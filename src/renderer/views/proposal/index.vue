            ...
            <div v-if="List != null">
                <div v-for="item in List" class="itemhoder" :key="item.id" @click="goitem(item)">
                    <Card>
                        <Row slot="title">
                            <Col span="24">
                                <h4 style=" word-break:break-all;">#{{ item.id }} {{ item.content.value.title }}</h4>
                            </Col>
                        </Row>
                        ...
                        <p>
                            <Tag v-if="item.proposal_status === 'Passed'" color="success">
                                {{ $t(`proposalsPage.typename.${item.proposal_status}`) }}
                            </Tag>
                            <Tag v-if="item.proposal_status === 'Rejected'" color="error">
                                {{ $t(`proposalsPage.typename.${item.proposal_status}`) }}
                            </Tag>
                            <Tag v-if="item.proposal_status === 'VotingPeriod'" color="warning">
                                {{ $t(`proposalsPage.typename.${item.proposal_status}`) }}
                            </Tag>
                            <Tag v-if="item.proposal_status === 'DepositPeriod'" color="primary">
                                {{ $t(`proposalsPage.typename.${item.proposal_status}`) }}
                            </Tag>
                            <Tag v-if="item.proposal_status === 'Failed'" color="default">
                                {{ $t(`proposalsPage.typename.${item.proposal_status}`) }}
                            </Tag>
