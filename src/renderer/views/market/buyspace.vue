<template>
    <div class="customer-container">
        <div class="tableContainer">
            <Row>
                <Col span="7">
                    <h3>{{ $t('marketpage.Optionaltitle') }}</h3>
                </Col>
            ...
            <Table
                ...
                :loading="loading"
                :columns="OrderListcolumnsisSort"
                :data="OrderList"
            >
                <template slot-scope="{ row, index }" slot="action">
                    <Button v-if="row.status == '0'" @click="openBuyingspace(row)" type="primary" size="small">
                        {{ $t('marketpage.selltable.Purchasespace') }}
                    </Button>
                </template>
        ...
        async getmarketlist() {
            ...
            try {
                ...
                if (res.state) {
                    ...
                    if (this.$store.getters.getselectMarket != this.$data.selectmarket.marketAddress) {
