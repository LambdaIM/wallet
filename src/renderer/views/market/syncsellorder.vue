<template>
    <div class="customer-container">
      ...
      {{$t('syncorderpage.no-data-text')}}
            ...
            <Table @on-sort-change="OrderListSort" :loading="loading" :columns="OrderListcolumns" :data="OrderList">
                <template slot-scope="{ row, index }" slot="action">
                    <Button :to="`/market/sellorderinfo/${row.orderId}`" type="primary" size="small">
                        {{ $t('marketpage.ordertable.orderdetails') }}
                    </Button>
                </template>
        ...
        async getmarketsyncTime() {
            ...
            try {
                ...
                if (res.state) {
                    ...
                    this.$data.syncTime = moment(res.data)
                        .locale(langType)
                        .fromNow();
                }
            } catch (error) {
