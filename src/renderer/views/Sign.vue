<template>
  <div class="customer-container">
       <Mycard :cardtitle="$t('Sign.t1')" class="mt20">
      <div class="content" slot="card-content">

        <Row class-name="card-item">
          <Col span="4" class-name="title-wrapper">
            <span class="title">{{$t('Sign.t2')}}:</span>
          </Col>
        ...
        <Row class-name="card-item">
          <Col span="4" class-name="title-wrapper">
            <span class="title">{{$t('Sign.t3')}}:</span>
          </Col>
          <Col span="20" class-name="content-wrapper">
        ...
        <Row class-name="card-item">
          ...
          <Col span="10" class-name="content-wrapper">
            <Button @click="Datacollection" type="primary">{{$t('Sign.t5')}}</Button>
          </Col>
        </Row>
        <Row class-name="card-item">
          <Col span="4" class-name="title-wrapper">
            <span class="title">{{$t('Sign.t4')}}:</span>
          </Col>
          <Col span="20" class-name="content-wrapper">
    ...
    <Modal
      ...
      >
        ...
        <div slot="footer">
          <Button  type="primary" @click="walletsign">{{$t('home.m2.t2')}}</Button>
        </div>
      </Modal>
  </div>
</template>
    ...
    walletsign:async function(){
      ...
      console.log(result)
       if(result.state==true){
         ...
         if (data.code == "1001") {
              this.$Notice.error({
                title: this.$t('home.action.t5')
              });
            } else {
              // _this.$data.passwordModal = false;
              this.$Notice.error({
                title: this.$t('home.action.t6')
              });
            }
