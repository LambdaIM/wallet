    ...
    <div>
        ...
        >
            <Form @keydown.native.enter.prevent="prewithdrawalLAMB">
                <p>{{ $t('Dialog.selectBuy.Marketname') }}：{{ market.name }}</p>
                <br />
                <p>
                    {{ $t('Dialog.selectBuy.Mineraddress') }}：
                    <a @click="openminerpage(orderinfo.address)">{{ orderinfo.address }}</a>
                </p>
                <br />
                <p>{{ $t('Dialog.selectBuy.Minimumspace') }}：{{ orderinfo.minBuySize }}GB</p>
                <br />
                <p>
                    {{ $t('Dialog.selectBuy.Minimumtime') }}：{{ orderinfo.minDuration | formatMonth
                    }}{{ $t('Dialog.selectBuy.month') }}
                </p>
                <br />
                <p>
                    {{ $t('Dialog.selectBuy.Maximumtime') }}：{{ orderinfo.maxDuration | formatMonth
                    }}{{ $t('Dialog.selectBuy.month') }}
                </p>
                <br />
                <p>{{ $t('Dialog.selectBuy.unitprice') }}：{{ orderinfo.price | Lambformat }}</p>
                <br />
                <p>{{ $t('Dialog.selectBuy.Odds') }}：{{ parseInt(orderinfo.rate) }}</p>
                <br />
                <p>{{ $t('Dialog.selectBuy.amountspace') }}：{{ orderinfo.sellSize }}GB</p>
                <br />
                <p>{{ $t('Dialog.selectBuy.remainingspace') }}：{{ orderinfo.unusedSize }}GB</p>
                <br />
                <p>
    ...
    methods: {
        ...
        prewithdrawalLAMB() {
            ...
            if (spaceDuration < this.$data.orderinfo.minDuration / this.$data.timeunit) {
                this.$Notice.warning({
                    title:
                        this.$t('Dialog.selectBuy.Minimumtime') +
            ...
            if (spaceDuration > this.$data.orderinfo.maxDuration / this.$data.timeunit) {
                this.$Notice.warning({
                    title:
                        this.$t('Dialog.selectBuy.Maximumtime') +
