<template>
    <div class="customer-container">
        <div class="tableContainer">
            <div v-if="isNaN(tbbday) != true" class="Earningstips">
                {{ $t('stakinginfo.Pledge1tbbmining') }}，{{ $t('stakinginfo.dayacquisition', [tbbday]) }}，{{
                    $t('stakinginfo.monthacquisition', [tbbmonth])
                }}，{{ $t('stakinginfo.yearsacquisition', [tbbyear]) }}
            </div>
            ...
            <Tabs>
                <TabPane :label="$t('staking.Mypledge')">
                    <Table v-if="mydelegationsList != null" size="large" :columns="columnsme" :data="mydelegationsList">
                        ...
                        <template slot-scope="{ row, index }" slot="operator_address">
                            <router-link :to="link(row.operator_address)" class="item">
                    ...
                    <Table v-else :loading="mydelegationsList == null"></Table>
                </TabPane>
                <TabPane :label="$t('staking.Validatorlist')">
                    ...
                    <Table :loading="validatorsList == null" v-else></Table>
                </TabPane>

                <TabPane :label="$t('staking.ExplainTab')">
                    <ul v-if="dataParameters != undefined" class="ulpanel">
                        <li>
                            {{ $t('staking.Explain.unbonding_time') }}:{{
                                (dataParameters.unbonding_time / (1000 * 1000 * 1000 * 60 * 60 * 24)).toFixed(2)
                            }}{{ $t('staking.Explain.unit') }}
                        </li>
                        <li>{{ $t('staking.Explain.max_validators') }}:{{ dataParameters.max_validators }}</li>
                        <li v-if="dataParameters.bond_denom != undefined">
                            {{ $t('staking.Explain.bond_denom') }}:{{ dataParameters.bond_denom.substring(1) }}
                        </li>
                    </ul>
                </TabPane>
            </Tabs>
...
export default {
            ...
            try {
                ...
                if (res.state) {
                    ...
                    if (this.$data.validatorsList instanceof Array) {
                        this.$data.validatorsList.sort(function(a, b) {
        ...
        async TBBYield() {
            try {
                ...
                if (mintingres.state && poolres.state && stakingres.state) {
                    ...
                    var ProfitProportion =
                        (1 - community_tax - base_proposer_reward - bonus_proposer_reward - pdp_reward) *
                        (1 - consensus_validator_fixed_commission_rate);

                    var NewlyLamb = mintingres.data.minting.replace(/\"/g, '');
                    var { bonded_tokens } = poolres.data;
                    var result = this.bigNum(1e6)
                        .div(bonded_tokens)
                        .times(NewlyLamb)
                        .times(ProfitProportion);
                    console.log(result);
    ...
    computed: {
        ...
        tbbday() {
            ...
            return this.bigNum(this.$data.tbbreturnlamb)
                .div(365)
                .div(1e6)
                .toFixed(2);
        },
