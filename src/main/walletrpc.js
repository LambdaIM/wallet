...
export default function() {
    ...
    eipc.answerRenderer('CreateSellOrder', async query => {
        ...
        try {
            var TxMessageload = await WM.TransferCreateSellOrder(
                marketName,
                price,
                rate,
                sellSize,
    ...
    eipc.answerRenderer('CreateBuyOrder', async query => {
        var { duration, size, sellOrderId, marketName } = query;
        ...
        try {
            var TxMessageload = await WM.TransferCreateBuyOrder(duration, size, sellOrderId, marketName);
    ...
    eipc.answerRenderer('AuthorizeMiningPubKey', async query => {
        ...
        if (user == undefined) {
            throw resultView(null, false, 'need user');
        }
        if (AssetName == undefined) {
            throw resultView(null, false, errorList.need_AssetName);
        }
        ...
        if (isAllowed == undefined) {
            throw resultView(null, false, 'need isAllowed');
        }

        try {
            var TxMessageload = await WM.AuthorizeMiningPubKey(user, AssetName, isAllowed);
            return resultView(TxMessageload, true);
        } catch (error) {
            throw resultView(null, false, error);
        }
    ...
    eipc.answerRenderer('assertUserdelegate', async query => {
        log.info('assertDeactivateMiner');

        var { AssetName, address, amount, delegateType } = query;
        console.log(AssetName, address);
        if (AssetName == undefined) {
            throw resultView(null, false, errorList.need_AssetName);
        }
        ...
        if (delegateType == undefined) {
            throw resultView(null, false, 'need delegateType');
        }

        try {
            var TxMessageload = await WM.DamUserDelegate(AssetName, address, amount, delegateType, '');
